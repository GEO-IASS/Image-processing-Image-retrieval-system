...
% Covariance Matrix method for image based retrieval system using generalized
% eigenvalues
% Extracts a 9 dimensional feature vector
% Performs image integrals for covariance matrix calculations
% Calculates distance between images using generalized eigenvalues method
...
for i = 1:length(cov_matrix)  
    ...
    for j = 1:length(cov_matrix)
        ...
        for k = 1:47
            distance(k,j) = covariance_matrix_distance(test_cov{1,k}, folder_sample{1,k});              
        end
    end
    % obtain the images with the shortest distance from the query image
    dis = sum(distance,1);
